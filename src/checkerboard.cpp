...
public:
    ...
    virtual T eval(const Point2f & uv) override {
        ...
        if ( int( std::floor( uv.x()/m_scale.x() + m_delta.x())) % int(2) == 0)  {
            if ( abs(int( std::floor( uv.y()/m_scale.y() + m_delta.y()))) % int(2) == 1) {
                return m_value2;
            }
        }
        if ( abs(int( std::floor( uv.x()/m_scale.x() + m_delta.x()))) % int(2) == 1)  {
            if ( int( std::floor( uv.y()/m_scale.y() + m_delta.y())) % int(2) == 0) {
                return m_value2;
            }
        }
        return m_value1;
